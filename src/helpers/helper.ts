...
export const isCommand = (command: string) => command.match(/^[LRF]+$/);
export const isOrientation = (orientation: string) => orientation.match(/^[NESW]$/);
export const isPosition = (position: Position) => position.X >= 0 && position.Y >= 0;
